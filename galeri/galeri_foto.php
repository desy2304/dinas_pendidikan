...
// ==== Bangun WHERE clause: halaman ini SELALU dikunci ke kategori = 'foto' ====
$whereParts = ["kategori = 'foto'"];
if ($filterBulan !== '') {
...
// ==== Statistik — selalu dari data kategori foto saja ====
$totalFoto = 0;
if ($r = mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM galeri WHERE kategori = 'foto'"))
    $totalFoto = mysqli_fetch_assoc($r)['jml'];

$fotoBulanIni = 0;
if ($r = mysqli_query($koneksi, "SELECT COUNT(*) AS jml FROM galeri WHERE kategori = 'foto' AND MONTH(tanggal) = MONTH(CURDATE()) AND YEAR(tanggal) = YEAR(CURDATE())"))
    $fotoBulanIni = mysqli_fetch_assoc($r)['jml'];

// Statistik foto bulan yang difilter (hanya muncul jika ada filter)
$fotoHasilFilter = 0;

// ==== Notifikasi ====
$notif = $_GET['notif'] ?? null;
$notifMsg = [
    'sukses_tambah' => ['type' => 'success', 'text' => 'Foto berhasil ditambahkan ke galeri.',    'icon' => 'fa-check-circle'],
    'sukses_edit'   => ['type' => 'success', 'text' => 'Data foto berhasil diperbarui.',           'icon' => 'fa-check-circle'],
    'sukses_hapus'  => ['type' => 'success', 'text' => 'Foto berhasil dihapus dari galeri.',       'icon' => 'fa-check-circle'],
    'gagal_kosong'  => ['type' => 'danger',  'text' => 'Judul, tanggal, dan gambar wajib diisi.',  'icon' => 'fa-exclamation-circle'],
    'gagal_simpan'  => ['type' => 'danger',  'text' => 'Gagal menyimpan data ke database.',        'icon' => 'fa-exclamation-circle'],
    'gagal_hapus'   => ['type' => 'danger',  'text' => 'Gagal menghapus foto.',                    'icon' => 'fa-exclamation-circle'],
    'not_found'     => ['type' => 'danger',  'text' => 'Data foto tidak ditemukan.',               'icon' => 'fa-exclamation-circle'],
];

// ==== Daftar galeri (kategori foto saja) dengan filter bulan ====
$daftarGaleri = [];
...
$fotoHasilFilter = $totalDataFilter;

// ==== Daftar bulan tersedia untuk dropdown (kategori foto saja) ====
$daftarBulanTersedia = [];
if ($r = mysqli_query($koneksi, "SELECT DISTINCT DATE_FORMAT(tanggal, '%Y-%m') AS bulan_key, DATE_FORMAT(tanggal, '%M %Y') AS bulan_label FROM galeri WHERE kategori = 'foto' ORDER BY bulan_key DESC")) {
    while ($row = mysqli_fetch_assoc($r))
...
<head>
    ...
    <title>Galeri Foto - Disdik Sumenep</title>
    ...
    <style>
        ...
        #tambahGaleriModal .modal-header,
        #editGaleriModal .modal-header { background-color: #162F55; color: #fff; }
        #tambahGaleriModal .modal-header .close,
        #editGaleriModal .modal-header .close { color: #fff; opacity: .85; text-shadow: none; }
        ...
        <li class="nav-item active">
            ...
            <div id="collapseTwo" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    <a class="collapse-item active" href="../galeri/galeri_foto.php">Foto</a>
    ...
    <div id="content">
        ...
        <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <div>
                    <h1 class="h3 mb-0 text-gray-800">Kelola Galeri &mdash; Foto</h1>
                ...
                <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm text-white"
                    ...
                    <i class="fas fa-plus fa-sm text-white"></i> Tambah Foto
                </a>
            </div>
                    ...
                    <div class="card border-left-primary shadow h-100 py-2">
                        <div class="card-body">
                            <div class="row no-gutters align-items-center">
                                <div class="col mr-2">
                                    <div class="text-xs font-weight-bold text-dark text-uppercase mb-1">Total Foto</div>
                        ...
                        <div class="card-body">
                            <div class="row no-gutters align-items-center">
                                <div class="col mr-2">
                                    ...
                                    <div class="h5 mb-0 font-weight-bold text-gray-800"><?= $fotoBulanIni ?></div>
                                </div>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3" style="border-left:4px solid #162F55;">
                    <h6 class="m-0 font-weight-bold text-dark">
                        <i class="fas fa-th-large mr-2" style="color:#162F55;"></i>Foto Kegiatan
                    </h6>
                </div>
                <div class="card-body">
                    ...
                    <div class="filter-bar">
                        <form method="GET" action="galeri_foto.php" id="formFilter">
                            <div class="row align-items-end">

                                <!-- Search -->
                                <div class="col-md-4 mb-2 mb-md-0">
                                    <label><i class="fas fa-search mr-1"></i>Cari judul foto</label>
                                ...
                                <!-- Reset -->
                                <div class="col-md-4">
                                    <a href="galeri_foto.php" class="btn btn-outline-secondary btn-block"
                        ...
                        <div class="d-flex align-items-center mt-3" style="gap:10px; flex-wrap:wrap;">
                            ...
                            <span class="badge-filter">
                                ...
                                <a href="galeri_foto.php" title="Hapus filter">&times;</a>
                            </span>
                            <?php endif; ?>
                            <span class="jumlah-hasil">
                                Menampilkan <strong><?= count($daftarGaleri) ?></strong> dari <strong><?= $fotoHasilFilter ?></strong> foto
                    ...
                    <div class="gallery-grid" id="galleryGrid">
                        <?php if (empty($daftarGaleri)): ?>
                        <div style="grid-column:1/-1;" class="text-center text-muted py-5">
                            <i class="fas fa-images fa-3x mb-3 d-block text-gray-300"></i>
                            <?= $labelBulanAktif
                                ? 'Tidak ada foto di bulan <strong>' . htmlspecialchars($labelBulanAktif) . '</strong>.'
                                : 'Belum ada foto di galeri. Klik <strong>Tambah Foto</strong> untuk menambahkan.' ?>
                        </div>
                        <?php else: ?>
                        ...
                        <div class="gallery-item"
                            ...
                            <div class="gallery-item-info">
                                ...
                                <div class="gallery-item-actions">
                                    ...
                                    <form method="POST" action="hapus_foto.php" style="display:inline;"
                                        onsubmit="return confirm('Yakin ingin menghapus foto \'<?= htmlspecialchars(addslashes($g['judul'])) ?>\'?');">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tambahGaleriModalLabel">Tambah Foto Galeri</h5>
                <button class="close" type="button" data-dismiss="modal"><span>&times;</span></button>
            </div>
            <form method="POST" action="proses_tambah_foto.php" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" name="kategori" value="foto">
                    <div class="form-group">
                        <label class="font-weight-bold small">Judul</label>
                        <input type="text" class="form-control" name="judul" placeholder="Contoh: Rakor Kepala Sekolah 2026" required>
                    </div>
                    <div class="form-group">
...
<div class="modal fade" id="editGaleriModal" tabindex="-1" role="dialog" aria-labelledby="editGaleriModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editGaleriModalLabel">Edit Foto Galeri</h5>
                <button class="close" type="button" data-dismiss="modal"><span>&times;</span></button>
            </div>
            <form method="POST" action="proses_edit_foto.php" enctype="multipart/form-data">
                <div class="modal-body">
