...
$total       = mysqli_num_rows(mysqli_query($conn, "SELECT id FROM galeri WHERE kategori = 'prestasi'"));
$total_pages = max(1, (int) ceil($total / $per_page));

$q = "SELECT * FROM galeri WHERE kategori = 'prestasi' ORDER BY tanggal DESC LIMIT $per_page OFFSET $offset";
    ...
    <div class="galeri-full-grid">
      <?php if ($r && mysqli_num_rows($r) > 0): ?>
        <?php while ($row = mysqli_fetch_assoc($r)):
          ...
          $ada = !empty($resolved_gambar) && !empty($gambar_path) && file_exists($gambar_path);
        ?>
        <div class="galeri-full-item reveal" <?php if ($ada): ?>onclick="openLightbox('<?= htmlspecialchars($gambar_url) ?>')"<?php endif; ?>>
        ...
        <?php endwhile; ?>
      <?php else: ?>
        <div class="karyawan-empty" style="grid-column:1/-1">Belum ada dokumentasi prestasi pada galeri ini.</div>
      <?php endif; ?>
    </div>

    <?php if ($total_pages > 1): ?>
    <div class="pagination">
      <a class="page-btn <?= $halaman <= 1 ? 'disabled' : '' ?>" href="?page=galeri_prestasi&p=<?= $halaman - 1 ?>">‹ Prev</a>
      <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <a class="page-btn <?= $i == $halaman ? 'active' : '' ?>" href="?page=galeri_prestasi&p=<?= $i ?>"><?= $i ?></a>
      <?php endfor; ?>
      <a class="page-btn <?= $halaman >= $total_pages ? 'disabled' : '' ?>" href="?page=galeri_prestasi&p=<?= $halaman + 1 ?>">Next ›</a>
    </div>
    <?php endif; ?>
  </div>
</section>